...
namespace MFST
{
    ...
    bool Mfst::start()
    {
        ...
        case LENTA_END:
        {
            MFST_TRACE4("------>LENTA_END")
                cout << "------------------------------------------------------------------------------------------------\n";
            sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: все строки %d, синтаксический анализ выполнен успешно", 0, lex.table[lex.size - 1].numberOfString);
            cout << setw(4) << left << 0 << "все строки " << lex.table[lex.size - 1].numberOfString << ", синтаксический анализ выполнен успешно\n";
            rc = true;
            break;
        }
    ...
    char* Mfst::getDiagnosis(short n, char* buf)
    {
        ...
        if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
        {
            ...
            sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: строка %d,%s", err.id, lex.table[lpos].numberOfString, err.message);
            rc = buf;
        }
        return rc;
    }
