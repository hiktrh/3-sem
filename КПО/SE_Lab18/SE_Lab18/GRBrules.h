...
namespace GRB
{
	Greibach greibach(NS('S'), TS('$'),				// начальный символ, или строка (дно стека)
		6,											// количество правил
		...
		Rule(NS('N'), GRB_ERROR_SERIES + 1,			// Неверный идентификатор
		...
		Rule(NS('E'), GRB_ERROR_SERIES + 2,			// Выражение в вычислении
		...
		Rule(NS('F'), GRB_ERROR_SERIES + 4,			// Формула в параметрах вычисляемой функции
			2,		// F	-> ti | ti,F
			Rule::Chain(2, TS('t'), TS('i')),
			Rule::Chain(4, TS('t'), TS('i'), TS(','), NS('F'))
		),
		Rule(NS('M'), GRB_ERROR_SERIES + 2,			// Выражение в вычислении
			2,		// M -> vE | vEM
