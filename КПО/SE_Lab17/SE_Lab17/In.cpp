	...
	for (int i = 0; i < in.size; i++)
	{
		if (in.text[i] == '\0')
		{
			...
			if (i == 0 && in.text[i]==' ')
			{
				...
				else if (in.text[i] == ' ' && (in.text[i + 1] == '(' || in.text[i + 1] == '{' || in.text[i + 1] == ')' || in.text[i + 1] == '}' || in.text[i + 1] == '=' || in.text[i + 1] == '+' || in.text[i + 1] == '-' || in.text[i + 1] == '*' || in.text[i + 1] == '/' || in.text[i + 1] == '%' || in.text[i + 1] == '\n' || in.text[i + 1] == '\0'))
				{
					in.countSpaces++;
					continue;
				}
				else if (in.text[i] == ' ' && (textWithoutSpaces[j - 1] == '(' || textWithoutSpaces[j - 1] == '{' || textWithoutSpaces[j - 1] == ')' || textWithoutSpaces[j - 1] == '}' || textWithoutSpaces[j - 1] == '=' || textWithoutSpaces[j - 1] == '+' || textWithoutSpaces[j - 1] == '-' || textWithoutSpaces[j - 1] == '*' || textWithoutSpaces[j - 1] == '/' || textWithoutSpaces[j - 1] == '%'))
				{
					in.countSpaces++;
					continue;
				}
